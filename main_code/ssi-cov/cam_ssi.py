...
# Create single setup
Pali_ss = SingleSetup(data, fs=120)
# Detrend and decimate
Pali_ss.filter_data(Wn= [1/ 60, 20/ 60] , order= 2, btype = 'bandpass') 
Pali_ss.detrend_data()
print(Pali_ss.fs)
Pali_ss.decimate_data(q=3, inplace=True) # q=decimation factor
print(Pali_ss.fs)#
...
ssicov = SSIcov_algo(name="SSIcov", br=200, ordmax=80)
...
fig3, ax3 = fsdd.plot_CMIF(freqlim=(1,20))
plt.show(block=True)
fig4, ax4 = ssicov.plot_STDiag(freqlim=(1,20), hide_poles=False)
plt.show(block=True)
# plot frequecy-damping clusters for SSI
fig5, ax5 = ssicov.plot_cluster(freqlim=(1,20))
plt.show(block=True)

# Select modes to extract from plots
Pali_ss.MPE_fromPlot("SSIcov", freqlim=(1,20))
